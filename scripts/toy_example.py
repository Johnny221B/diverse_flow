...
def draw_means(ax, means: torch.Tensor):
    m = means.cpu().numpy()
    ax.scatter(m[:, 0], m[:, 1], marker='+', s=200, c='black', alpha=1.0, linewidths=1.5, zorder=10)
...
def subplot_snapshot(ax, x0: torch.Tensor, x_now: torch.Tensor,
    ...
    # 初始点（浅色）
    _xi = xi.cpu().numpy()
    ax.scatter(_xi[:, 0], _xi[:, 1], s=9, alpha=0.5, marker='o', c='lightblue')
    ...
    # 当前点
    _xT = xT.cpu().numpy()
    ax.scatter(_xT[:, 0], _xT[:, 1], s=28, alpha=0.6, marker='x', c='orange')
