...
"""
Diverse FLUX.1-schnell backbone
(基于 SD3.5 版本的多样性注入逻辑，替换 base model 为 FLUX.1-schnell)

特点：
- 使用 diffusers.FluxPipeline
- 仍然：
  * VAE 卡上 decode
  * CLIP 体积损失 + VJP 回 latent
  * Heun 风格 endpoint 预测
  * A+B 噪声 + 批内正交初噪

用法示例：

python flux_backbone.py \
  --prompt "a close-up photo of a truck" \
  --G 8 --steps 4 --guidance 3.0 --seed 1111 \
  --model-dir ../models/FLUX.1-schnell \
  --device-transformer cuda:0 --device-vae cuda:0 --device-clip cuda:0 \
  --method flux

注意：
- FLUX 官方推荐 steps 很少（1~8），schnell 常用 4 步左右。
- 高分辨率时 VRAM 压力会很大，建议先从 512x512 或 768x768 试起。
"""
...
def print_mem_all(tag: str, devices):
    lines = [f"[{tag}]"]
    for d in devices:
        if d.type == "cuda":
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Diverse FLUX.1-schnell backbone (volume + orthogonal noise)"
    )
    ...
    ap.add_argument("--steps", type=int, default=4)  # schnell 通常很小
    ap.add_argument("--guidance", type=float, default=3.0)
    ap.add_argument("--seed", type=int, default=1111)

    # 模型路径（本地 HF diffusers 目录）
    ap.add_argument(
        "--model-dir",
        type=str,
        default=os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "models", "FLUX.1-schnell")
        ),
    )
    ap.add_argument(
        "--clip-jit",
    ...
    ap.add_argument("--method", type=str, default="flux")

    # 多样性 / 体积相关
    ap.add_argument("--gamma0", type=float, default=0.02)
    ap.add_argument("--gamma-max-ratio", type=float, default=0.10)
    ap.add_argument("--partial-ortho", type=float, default=0.95)
    ap.add_argument("--t-gate", type=str, default="0.7,0.95")
    ...
    ap.add_argument(
        "--noise-timing", type=str, default="early", choices=["early", "late"]
    )
...
def main():
    ...
    try:
        ...
        # ===== 1) 加载 FLUX pipeline 到 CPU =====
        model_dir = _resolve_model_dir(args.model_dir)
        _log("Loading FLUX.1-schnell (CPU) ...", args.debug)
        ...
        cfg = DiversityConfig(
            ...
            clip_image_size=224,
            leverage_alpha=0.0,  # 建议先 0，避免奇异矩阵
        )
        ...
        def _inject_init_ortho(latents: torch.Tensor, strength: float, mode: str = "blend"):
        ...
        def diversity_callback(ppl, i, t, kw):
            ...
            for s in range(0, B, chunk):
                ...
                g_proj = (
                    project_partial_orth(grad_lat, v_est, cfg.partial_ortho)
                    if v_est is not None
                    else grad_lat
                )
                ...
                target_brown = torch.full_like(base_norm, max(base_brown, 0.0))
                ...
                if (v_est is None) or (vnorm is None) or (
                ...
                xi_norm = _bn(xi_eff)
                noise_disp = xi_eff / (xi_norm.view(-1, 1, 1, 1) + 1e-12) * target.view(
                    -1, 1, 1, 1
                )
                ...
                scale = torch.minimum(
                    torch.ones_like(disp_cap), disp_cap / (div_raw + 1e-12)
                )

                delta_chunk = (gamma_sched * scale.view(-1, 1, 1, 1)) * div_disp + noise_disp

                delta_tr = (
                    delta_chunk.to(lat_new.device, non_blocking=True).to(lat_new.dtype)
                )
                ...
                if args.debug and (s == 0):
                    with torch.no_grad():
                        ...
                        print(
                            f"[t={t_norm:.2f}] |base|={n_base:.4f} |div|={n_div:.4f} "
                            f"|noise|={n_noise:.4f} gamma={gamma_sched:.4f} "
                            f"brown={brown_std:.4f} rho_t={rho_t:.3f}"
                        )
            ...
            if "ctrl_cache" in state:
                state["prev_ctrl_vae_cpu"] = torch.cat(
                    state["ctrl_cache"], dim=0
                ).to("cpu")
        ...
        # ===== 5) 调 Flux pipeline，拿 latent =====
        generator = (
            torch.Generator(device=dev_tr) if dev_tr.type == "cuda" else torch.Generator()
        )
        generator.manual_seed(args.seed)

        _log("Start Flux pipeline() ...", args.debug)
        latents_out = pipe(
            prompt=args.prompt,
            height=args.height,
            width=args.width,
        ...
        with torch.inference_mode(), torch.backends.cudnn.flags(
            ...
            images = checkpoint(
                lambda z: (
                    pipe.vae.decode(z / sf, return_dict=False)[0]
                    .float()
                    .clamp(-1, 1)
                    + 1.0
                )
