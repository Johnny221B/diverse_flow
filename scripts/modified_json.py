...
def parse_args():
    ...
    # 多个 seed / guidance
    ap.add_argument('--seeds', type=str, default="1111,2222,3333",
...
def main():
    ...
    try:
        ...
        def _sync_all_cuda():
            """同步所有相关 CUDA 设备，避免异步 kernel 污染计时"""
            ...
            for prompt in prompts:
                ...
                for guidance in guidance_list:
                    for seed in seed_list:
                        ...
                        # 计时前先 sync，一次 run 的时间窗口只包含 pipeline+decode(+profiler)
                        _sync_all_cuda()
                        t0_run = time.perf_counter()

                        # FLOPs profiling（可选，只包 pipeline+decode）
                        ...
                        # 读取多卡峰值显存（只看 pipeline+decode 阶段）
                        if mem_devices and torch.cuda.is_available():
                            peaks_mb = []
                            for d in mem_devices:
                                try:
