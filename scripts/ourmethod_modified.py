...
def parse_args():
    ...
    ap.add_argument('--t-gate', type=str, default='0.85,0.99')     # 仅用于确定性体积漂移
    ...
    # 噪声控制（A+B）
    ap.add_argument('--eta-sde', type=float, default=0.5, help='SDE/Brownian 噪声强度 (与 scheduler 对齐)')
    ...
    try:
        ...
        def diversity_callback(ppl, i, t, kw):
            ...
            for s in range(0, B, chunk):
                e = min(B, s + chunk)
                # 当前 step 的 latent（在 VAE 卡上）—— autograd 变量
                z = lat_vae_full[s:e].detach().clone().requires_grad_(True)

                # ===== NEW: 基流速度估计（用于 Heun 预测 + 正交投影）=====
                v_est = None
                ...
                # ===== NEW: Heun 风格的 endpoint 预测 latent =====
