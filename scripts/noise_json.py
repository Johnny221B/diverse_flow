...
#         # ===== 1) CPU 加载，再手动上卡 =====
#         model_dir = _resolve_model_dir(args.model_dir)  # NOTE: if you had a typo, fix to args.model_dir
#         # 如果上面这一行有报错，把 model-dir 改成 model_dir：
...
#                     # 单步新增方差：sigma_cur^2 - sigma_next^2
...
def main():
    ...
    try:
        ...
        def diversity_callback(ppl, i, t, kw):
            ...
            # 噪声时间权重：在 noise_gate 区间内从 1 线性衰减到 0
            ng0, ng1 = noise_gate
            if t_norm <= ng0:
                noise_w = 0.0
            elif t_norm >= ng1:
                ...
                # t_norm 从 ng1→ng0 下降，权重从 1→0，
                # 这里按「靠近 early(t_norm≈1) 权重大」来写
