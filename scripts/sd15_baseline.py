...
"""
SD1.5 baseline (no diversity pipeline), JSON-driven prompts.

- Backbone: runwayml/stable-diffusion-v1-5 (可通过 --model-dir 改)
- 从 JSON spec 里按 concept 读取若干 prompt：
    {
        "bowl": [
            "a photo of a bowl",
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="SD1.5 baseline with JSON-driven prompts (no diversity pipeline)"
    )
    ...
    ap.add_argument(
        ...
        type=str,
        required=True,
        help="JSON 文件路径，例如 /data2/toby/specs/prompt.json，结构如 {\"bowl\": [\"a photo of a bowl\", ...]}",
    )
    ap.add_argument(
        "--concepts",
        type=str,
    ...
    # 生成参数
    ap.add_argument("--G", type=int, default=64, help="每个 prompt 生成的图片数")
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help='多个 guidance，逗号分隔，如 "3.0,5.0,7.5"，不填则用 --guidance',
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help='多个 seed，逗号分隔，如 "1111,2222,3333"，不填则用 --seed',
    )
    ...
    ap.add_argument(
        ...
        help="本地路径或 HF repo id（如 runwayml/stable-diffusion-v1-5）",
    )
...
def main():
    ...
    try:
        ...
        _log("Loading StableDiffusionPipeline (SD1.5) ...", args.debug)
        pipe = StableDiffusionPipeline.from_pretrained(
            model_id_or_path,
            torch_dtype=dtype,
            safety_checker=None,  # 省显存
            ...
            for prompt in prompts:
                ...
                for seed in seeds:
                    for g in guidances:
                        ...
                        _log(
                            f"    Start SD1.5 sampling (concept={concept}, "
