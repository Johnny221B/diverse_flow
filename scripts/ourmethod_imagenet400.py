...
输出：
- 普通模式：outputs/<method>_<prompt_slug>/{imgs,eval}/
- ImageNet-400 模式（--imagenet-json）：outputs/imagenet_400/<method>/cls_XXX.png
"""
...
def parse_args():
    ...
    ap.add_argument('--prompt', type=str, default=None,
                    help='单次生成的文本 prompt；若使用 --imagenet-json，可留空')
    ap.add_argument('--negative', type=str, default='')
    ap.add_argument('--G', type=int, default=1, help='普通模式下每个 prompt 生成 G 张图')
    ...
    # ImageNet-400 模式：读取 JSON
    ap.add_argument('--imagenet-json', type=str, default=None,
                    help='若提供，则从 json 中读取 {class_id: prompt}，对每个 prompt 生成一张图')
...
def main():
    ...
    # 至少要有 prompt 或 imagenet-json 之一
    if args.imagenet_json is None and (args.prompt is None or len(args.prompt.strip()) == 0):
        print("ERROR: either --prompt or --imagenet-json must be provided.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        def reset_state():
            """每次重新跑一个 prompt 时重置 callback 状态。"""
        ...
        # ---------- 模式 A：ImageNet-400，从 json 读取 400 个 prompt ----------
        if args.imagenet_json is not None:
            # 加载 json：{ "0": "a photo of a tench", ... }
            with open(args.imagenet_json, "r", encoding="utf-8") as f:
                imagenet_prompts: Dict[str, str] = json.load(f)

            # 输出目录：outputs/imagenet_400/<method>/
            out_dir = os.path.join(outputs_root, "imagenet_400", args.method)
            os.makedirs(out_dir, exist_ok=True)
            _log(f"[ImageNet-400] Output dir: {out_dir}", True)

            # 遍历每个 class_id / prompt
            # 这里严格按照 class_id 排序，保证 cls_000 ~ cls_399
            for cls_id_str, prompt in sorted(imagenet_prompts.items(), key=lambda kv: int(kv[0])):
                cls_id = int(cls_id_str)
                reset_state()

                # per-class seed
                generator = torch.Generator(device=dev_tr) if dev_tr.type == 'cuda' else torch.Generator()
                generator.manual_seed(args.seed + cls_id)

                _log(f"[ImageNet-400] class {cls_id:03d}: prompt = {prompt}", True)

                # 这里只生成一张图：num_images_per_prompt = 1
                latents_out = pipe(
                    prompt=prompt,
            ...
            _log("[ImageNet-400] Done. All images generated.", True)
            return

        # ---------- 模式 B：原始单 prompt + G 张图 ----------
        prompt_slug = _slugify(args.prompt or "")
