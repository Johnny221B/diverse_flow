...
仅改动：
- 新增 --spec 读取 DIMCIM 的 dense_prompts JSON；
- 输出路径改为 <outputs_root>/{method}_{concept}_CIMDIM/{coarse_imgs|dense_imgs|eval}/{prompt_slug}/00.png..；
- 单 prompt (--prompt) 仍然可用（落到 {method}_{single}_CIMDIM/dense_imgs/...）。
"""
...
# === DIMCIM: 解析 dense_prompts JSON ===
def _parse_dimcim_spec(path: str) -> Tuple[str, List[str], List[str]]:
    """
    期望结构：
    {
      "concept": "bus",
    ...
    返回：(concept, coarse_prompts_unique, dense_prompts_unique)
    """
    import json
    ...
    if not isinstance(rows, list):
        raise ValueError("DIMCIM JSON: 'coarse_dense_prompts' must be a list.")
    for r in rows:
...
def parse_args():
    ...
    # === DIMCIM：新增 spec；--prompt 作为后备 ===
    ap.add_argument('--spec', type=str, default=None,
                    help='DIMCIM dense_prompts JSON；若提供，将忽略 --prompt 并对所有 coarse/dense 逐一生成')
    ap.add_argument('--prompt', type=str, default=None,
                    help='单条 prompt（仅当未提供 --spec 时使用；将视为 dense 类别）')
    ...
    ap.add_argument('--out', type=str, default=None,
                    help='outputs 根目录（若不设，则默认 <project_root>/outputs）')
...
def main():
    ...
    try:
        ...
        # ===== 4) 输出根目录（DIMCIM 结构） =====
        outputs_root = args.out.strip() if (args.out and len(args.out.strip()) > 0) else os.path.join(project_root, 'outputs')

        # === 把一次“生成并保存”封装成函数（不改采样逻辑） ===
        def generate_and_save(prompt_text: str, target_dir: str):
            os.makedirs(target_dir, exist_ok=True)
            eval_dir = os.path.join(os.path.dirname(os.path.dirname(target_dir)), "eval")
            ...
            _log(f"Start pipeline() ... prompt={prompt_text}", args.debug)
            latents_out = pipe(
                prompt=prompt_text,
            ...
            # 严格按 DIMCIM 命名：00.png, 01.png, ...
            from torchvision.utils import save_image
            zpad = max(2, len(str(max(0, args.G - 1))))  # 至少两位
            for i in range(images.size(0)):
                save_image(images[i].cpu(), os.path.join(target_dir, f"{i:0{zpad}d}.png"))

            _log(f"Saved {images.size(0)} images -> {target_dir}", True)

        # ===== 5) 根据 --spec 或 --prompt 决定生成哪些 prompt，并落盘到 DIMCIM 结构 =====
        if args.spec:
            concept, coarse_list, dense_list = _parse_dimcim_spec(args.spec)

            base_dir = os.path.join(outputs_root, f"{args.method}_{concept}_CIMDIM")
            coarse_root = os.path.join(base_dir, "coarse_imgs")
            dense_root  = os.path.join(base_dir, "dense_imgs")
            ...
            # Coarse
            for cp in coarse_list:
                pslug = _slugify(cp)
                out_dir = os.path.join(coarse_root, pslug)
                print(f"[DIMCIM] Coarse: '{cp}' -> {out_dir}")
                generate_and_save(cp, out_dir)

            # Dense
            for dp in dense_list:
                pslug = _slugify(dp)
                out_dir = os.path.join(dense_root, pslug)
                print(f"[DIMCIM] Dense:  '{dp}' -> {out_dir}")
                generate_and_save(dp, out_dir)

        else:
            # 单条 prompt 模式：也按 DIMCIM 的 dense_imgs 放
            if not args.prompt:
                raise ValueError("Provide --spec (DIMCIM JSON) or --prompt (single prompt).")
            concept = "single"
            base_dir = os.path.join(outputs_root, f"{args.method}_{concept}_CIMDIM")
            dense_root = os.path.join(base_dir, "dense_imgs")
            os.makedirs(dense_root, exist_ok=True)
            ...
            pslug = _slugify(args.prompt)
            out_dir = os.path.join(dense_root, pslug)
            print(f"[DIMCIM] Single (dense): '{args.prompt}' -> {out_dir}")
            generate_and_save(args.prompt, out_dir)
