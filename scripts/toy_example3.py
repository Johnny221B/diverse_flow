# -*- coding: utf-8 -*-
"""
Flow Matching vs. OSCAR on 3x3 GMM (2x3 figure)

- Row 1: Standard FM
- Row 2: OSCAR
- Col 1: Step 400/2000, with init points + trajectories
- Col 2: Step 1200/2000, only orange particles + 1-sigma circles
- Col 3: Step 2000/2000, only orange particles + 1-sigma circles
"""
...
class GMM2D:
    ...
    def grid(grid=3, span=(-2, 2), sigma=0.22):
        ...
        w = np.ones(mus.shape[0], dtype=np.float32)
        w = w / w.sum()
...
def fm_loss(model, x0, y_onehot, p_drop=0.2):
    B = x0.shape[0]
    z = torch.randn_like(x0)          # 训练里参考分布 N(0, I)
...
def integrate_oscar_balanced(
    ...
    gamma0: float = 0.22, t_gate: Tuple[float, float] = (0.18, 0.85),
    sched_shape: str = 'sin2',
    partial_ortho: float = 1.0, repel_sigma: float = 0.26,
    repulsion_weight: float = 1.1,
    dens_alpha: float = 0.8, swirl_coef: float = 0.20,
    gamma_max_ratio: float = 0.35, r_cut_mult: float = 1.0,
    dist_tau_mult: float = 0.40,
    micro_every: int = 10, micro_step: float = 0.18,
    micro_iters: int = 1,
):
    dev = next(model.parameters()).device
    n = y_onehot.size(0)
    ...
    for i in range(steps):
        ...
        dens_scale = ((dens / (dens.mean() + 1e-12)).clamp_min(1e-3)
                      ).pow(dens_alpha).view(-1, 1)
        dist = torch.cdist(x, means).min(dim=1).values
        r_cut = r_cut_mult * max(float(repel_sigma), float(gmm_sigma))
        gate_dist = torch.sigmoid(
            (r_cut - dist) / (dist_tau_mult * r_cut + 1e-12)
        ).view(-1, 1)
        g_rep = repulsion_weight * g_rep * dens_scale * gate_dist

        # Orthogonal correction + swirl
        ...
        if micro_every > 0 and (i + 1) % micro_every == 0:
            x = micro_polish(x, r_min=0.6 * repel_sigma,
                             step=micro_step, iters=micro_iters)
...
def compute_line_alpha(x0: torch.Tensor, xT: torch.Tensor,
    ...
    if mode == 'density':
        ...
        a = torch.full((n,), float(np.clip(base, min_alpha, max_alpha)),
                       device=x0.device)
...
def draw_means(ax, means: torch.Tensor):
    m = means.cpu().numpy()
    ax.scatter(m[:, 0], m[:, 1], marker='+', s=200,
               c='black', alpha=1.0, linewidths=1.5, zorder=10)


def draw_sigma_circles(ax, means: torch.Tensor,
                       sigma: float, k_sigma: float = 1.0):
    """在每个 GMM mean 周围画 k_sigma * sigma 的圆，表示目标方差"""
    m = means.cpu().numpy()
    for (mx, my) in m:
...
def subplot_snapshot(
    ax,
    x0: torch.Tensor,
    x_now: torch.Tensor,
    line_alpha_mode='density',
    max_lines: Optional[int] = None,
    min_alpha=0.02,
    max_alpha=0.40,
    show_init: bool = True,
    show_lines: bool = True,
    init_color: str = 'lightblue',
    ...
    if show_init:
        _xi = xi.cpu().numpy()
        ax.scatter(_xi[:, 0], _xi[:, 1],
                   s=9, alpha=init_alpha,
                   marker=init_marker, c=init_color)

    # 轨迹连线
    if show_lines:
        alphas = compute_line_alpha(
            xi, xT, mode=line_alpha_mode,
        ...
        if (max_lines is not None) and (n > max_lines):
            idxs = np.random.default_rng(0).choice(
                n, size=max_lines, replace=False
            )

        segs = np.stack(
            [xi[idxs].cpu().numpy(), xT[idxs].cpu().numpy()], axis=1
        )
        cols = np.zeros((segs.shape[0], 4), dtype=np.float32)
        cols[:, 3] = alphas[idxs]
        lc = LineCollection(
            segs, colors=cols,
            linewidths=0.7, linestyles='--', antialiased=True
        )
        ax.add_collection(lc)

    # 当前粒子（橙色叉叉）
    _xT = xT.cpu().numpy()
    ax.scatter(_xT[:, 0], _xT[:, 1],
               s=28, alpha=0.6,
               marker='x', c='orange')
...
def main():
    ...
    # 推理（保留完整轨迹）
    STEPS = 2000
    print("Integrating Standard FM...")
    x_base_final, traj_base = integrate_flowmatch(
        model, y_onehot, x0=z0, steps=STEPS
    )
    print("Integrating OSCAR...")
    x_osc_final, traj_oscar = integrate_oscar_balanced(
        model, y_onehot, x0=z0, steps=STEPS,
        means=means, gmm_sigma=float(gmm.sigma),
        gamma0=0.22, t_gate=(0.18, 0.85),
        sched_shape='sin2',
        partial_ortho=1.0, repel_sigma=0.26,
        repulsion_weight=1.1,
        dens_alpha=0.8, swirl_coef=0.20,
        gamma_max_ratio=0.35, r_cut_mult=1.0,
        dist_tau_mult=0.40,
        micro_every=10, micro_step=0.18,
        micro_iters=1
    )

    # 选择三个快照（early / mid / final）
    snap_fracs = (0.2, 0.6, 1.0)   # 400/2000, 1200/2000, 2000/2000
    ...
    fig.suptitle(
        "Flow Matching vs. OSCAR — Progress Over Time",
        fontsize=20, y=0.99
    )
    ...
    for c, idx in enumerate(snap_indices):
        if c == 0:
            # 第一列：早期 step，蓝点 + 轨迹 + 橙色
            draw_means(axes[0, c], means)
            subplot_snapshot(
                axes[0, c], z0, traj_base[idx],
                line_alpha_mode='density',
                show_init=True, show_lines=True
            )
            draw_means(axes[1, c], means)
            subplot_snapshot(
                axes[1, c], z0, traj_oscar[idx],
                line_alpha_mode='density',
                show_init=True, show_lines=True
            )

        elif c == 1:
            # 第二列：中期 step，只橙色粒子 + 1σ 圆
            draw_means(axes[0, c], means)
            draw_sigma_circles(axes[0, c], means, sigma=gmm.sigma, k_sigma=1.0)
            subplot_snapshot(
                axes[0, c], z0, traj_base[idx],
                line_alpha_mode='density',
                show_init=False, show_lines=False
            )

            draw_means(axes[1, c], means)
            draw_sigma_circles(axes[1, c], means, sigma=gmm.sigma, k_sigma=1.0)
            subplot_snapshot(
                axes[1, c], z0, traj_oscar[idx],
                line_alpha_mode='density',
                show_init=False, show_lines=False
    ...
    png = os.path.join(out_dir, "gmm_grid2x3_sigma.png")
    pdf = os.path.join(out_dir, "gmm_grid2x3_sigma.pdf")
