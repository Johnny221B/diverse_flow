...
def parse_args():
    ap = argparse.ArgumentParser(
        description='Robust Noise studies for Diverse SD3.5 (no-embeds)'
    )

    ap.add_argument('--study', type=str, default='noise_gate',
                    choices=['noise_gate', 'noise_scale'],
                    help='选择实验类型：noise_gate 或 noise_scale')
    ...
    # [noise_gate] 噪声门控列表：形如 "0.0:0.95,0.5:0.95,0.6:0.95,..."
    ap.add_argument('--noise-gates', type=str,
                    default='0.0:0.95,0.5:0.95,0.6:0.95,0.7:0.95,0.8:0.95,0.9:0.95')

    # [noise_scale] 噪声尺度列表：形如 "0.5,0.75,1.0,1.25,1.5"
    ap.add_argument('--noise-scales', type=str, default='0.25,0.50,1.0,2.0,5.0,10.0')
...
def _parse_noise_scales(spec: str) -> List[float]:
    vals = []
    for tok in spec.split(','):
        tok = tok.strip()
        if not tok:
...
def main():
    ...
    try:
        ...
        # ===== 回调生成器 =====
        def diversity_callback_builder(cfg, noise_gain: float = 1.0):
            # noise_gain 统一缩放噪声 target 幅度（noise scale 实验）
            def diversity_callback(ppl, i, t, kw):
                ...
                for s in range(0, B, chunk):
                    ...
                    # —— 噪声尺度：统一缩放 target ——
                    if noise_gain != 1.0:
                        target = target * float(noise_gain)
        ...
        # ===== 输出根目录 =====
        if args.study == 'noise_gate':
            study_root = os.path.join(project_root, 'outputs', 'add_robust_study', 'noise_gate')
        else:
            study_root = os.path.join(project_root, 'outputs', 'add_robust_study', 'noise_scale')
        imgs_root = os.path.join(study_root, 'imgs')
        ...
        if args.study == 'noise_gate':
            ...
            for seed in seeds:
                for (t0, t1) in gate_pairs:
                    ...
                    callback = diversity_callback_builder(cfg, noise_gain=1.0)

                    leaf = f"seed{seed}_noisegate{t0:.1f}_{t1:.2f}"
                    out_dir = os.path.join(imgs_root, leaf)
        ...
        else:  # noise_scale study
            scales = _parse_noise_scales(args.noise_scales)
            # 固定一个典型 t_gate（与原默认一致）
            default_gate = (0.85, 0.95)
            for seed in seeds:
                for scale in scales:
                    cfg = DiversityConfig(**{**base_cfg_kwargs, "t_gate": default_gate})
                    if hasattr(vol, "update_config"):
                        vol.update_config(cfg)
                    callback = diversity_callback_builder(cfg, noise_gain=float(scale))

                    leaf = f"seed{seed}_noisescale{scale}"
                    out_dir = os.path.join(imgs_root, leaf)
                    os.makedirs(out_dir, exist_ok=True)
                    ...
                    _log(f"Start pipeline() | seed={seed} | noise_scale={scale}", True)
                    latents_out = pipe(
                        prompt=args.prompt,
                        negative_prompt=(args.negative if args.negative else None),
                        height=args.height, width=args.width,
