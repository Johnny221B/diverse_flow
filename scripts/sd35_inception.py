...
"""
Diverse SD3.5 (Inception-v3 encoder, no-embeds, TE=Transformer device, METHOD-CONSISTENT)
[with A+B noise + safe init-orthogonal injection]

本版将体积目标的图像编码器由 CLIP 替换为 Inception v3。
- New: --inception-ckpt 指向本地 Inception v3 权重（你们已下载）
- 体积特征尺寸改为 299（cfg.clip_image_size=299；字段名保持不变）
- 其它流程保持不变
...
def parse_args():
    ap = argparse.ArgumentParser(
        description='Diverse SD3.5 (Inception v3 encoder, METHOD-CONSISTENT)'
    )
    ...
    # Inception v3 本地权重
    ap.add_argument('--inception-ckpt', type=str, default=None,
                    help='本地 Inception v3 权重路径（.pth/.pt），若为 None 则用随机初始化（不建议）')
...
def main():
    ...
    try:
        ...
        cfg = DiversityConfig(
            ...
            sched_shape=args.sched_shape, clip_image_size=299,  # 关键：Inception 输入 299
            leverage_alpha=0.5,
        )
